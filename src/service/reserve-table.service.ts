...
import { SortColumnReserve, SortDirectionReserve } from 'src/app/directives/sortable-reserve.directive';
...
export interface StateReserve {
    ...
    pageSize: number;
    searchTerm: string;
    sortColumn: SortColumnReserve;
    sortDirection: SortDirectionReserve;
}

const compare = (v1: string | number | VehicleType, v2: string | number | VehicleType) => v1 < v2 ? -1 : v1 > v2 ? 1 : 0;

function sort(reserves: Reserve[], column: SortColumnReserve, direction: string): Reserve[] {
    if (direction === '' || column === '') {
        return reserves;
    } else {
...
export class ReserveTableService {
    ...
    set sortColumn(sortColumn: SortColumnReserve) { this._set({ sortColumn }); }
    set sortDirection(sortDirection: SortDirectionReserve) { this._set({ sortDirection }); }
